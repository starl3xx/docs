...
| component | weight | normalization | reference_constant |
|---|---:|---|---|
| velocity | 0.30 | fixed_reference | VELOCITY_REFERENCE |
| effectiveBuyers | 0.15 | fixed_reference | EFFECTIVE_BUYERS_REFERENCE |
| stickyLiquidity | 0.30 | fixed_reference | STICKY_LIQUIDITY_REFERENCE |
| retention | 0.15 | already_zero_to_one | n/a |
| momentum | 0.00 | n/a (gated off) | n/a |
| holderConcentration | 0.10 | already_zero_to_one | n/a |
...
The full bundle is exposed at [`/scoring/weights`](https://api.filter.fun/scoring/weights) under `response.constants` so an external auditor can fetch the live engine's parameters directly. Drift between the endpoint response and the published constants is a deploy bug, not an ambiguity.
...
Two concrete actions for an external auditor:
...
Expected response includes `version`, `weights`, `constants`, `flags`, `compositeScale`. Cross-check `version` against the active value below and the `constants` table above.
...
*HP scoring is the load-bearing mechanic of filter.fun's tournament. It's empirically grounded, cross-validated, formula-locked at named constants, and runtime-revisable as the protocol learns from its own data.*

**Cross-links:** [Settlement security model](/security/settlement) · [Audit status](/security/audit-status) · [Weights endpoint](https://api.filter.fun/scoring/weights)
