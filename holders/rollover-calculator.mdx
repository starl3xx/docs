...
export const RolloverCalculator = () => {
  ...
  const btn = { flex: 1, fontSize: '13px', padding: '8px 12px', background: 'transparent', border: '0.5px solid rgba(0,0,0,0.15)', borderRadius: '8px', cursor: 'pointer' };
  const row = { display: 'grid', gridTemplateColumns: '220px 1fr 90px', alignItems: 'center', gap: '12px' };
  const lbl = { fontSize: '14px', color: '#5f5e5a' };
  const out = { fontSize: '14px', fontWeight: 500, textAlign: 'right' };
  const card = { background: '#f5f5f5', padding: '14px', borderRadius: '8px' };
  const small = { background: '#f5f5f5', padding: '10px', borderRadius: '8px' };
    ...
    <div style={{ padding: '1rem 0', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      ...
      <div style={{ marginBottom: '1rem' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'baseline', gap: '12px', padding: '12px 16px', background: '#f5f5f5', borderRadius: '8px' }}>
          ...
          <span style={{ fontSize: '13px', color: '#5f5e5a' }}>{fmtUsd(totalFilteredLp * ethPrice)}</span>
          <span style={{ fontSize: '12px', color: '#888780', marginLeft: 'auto' }}>After 2.5% bounty: {fmtUsd(netPot * ethPrice)}</span>
        </div>
      </div>
      ...
      <div style={{ marginBottom: '1.5rem' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '8px' }}>
          <div style={small}><div style={{ fontSize: '11px', color: '#5f5e5a' }}>Rollover 45%</div><div style={{ fontSize: '14px', fontWeight: 500, marginTop: '2px' }}>{fmtEth(rollover)}</div></div>
          <div style={small}><div style={{ fontSize: '11px', color: '#5f5e5a' }}>Hold bonus 25%</div><div style={{ fontSize: '14px', fontWeight: 500, marginTop: '2px' }}>{fmtEth(holdBonus)}</div></div>
          <div style={small}><div style={{ fontSize: '11px', color: '#5f5e5a' }}>Mechanics 10%</div><div style={{ fontSize: '14px', fontWeight: 500, marginTop: '2px' }}>{fmtEth(mechanics)}</div></div>
          <div style={small}><div style={{ fontSize: '11px', color: '#5f5e5a' }}>POL 10%</div><div style={{ fontSize: '14px', fontWeight: 500, marginTop: '2px' }}>{fmtEth(pol)}</div></div>
          <div style={small}><div style={{ fontSize: '11px', color: '#5f5e5a' }}>Treasury 10%</div><div style={{ fontSize: '14px', fontWeight: 500, marginTop: '2px' }}>{fmtEth(treasury)}</div></div>
        </div>
      </div>
      ...
      <div style={{ borderTop: '0.5px solid rgba(0,0,0,0.15)', paddingTop: '1.25rem' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
          <div style={{ background: '#fbeaf0', padding: '14px', borderRadius: '8px' }}>
            ...
            <div style={{ fontSize: '22px', fontWeight: 500, color: '#4b1528' }}>{fmtEth(yourRolloverEth)}</div>
            <div style={{ fontSize: '13px', color: '#993556', marginTop: '2px' }}>{fmtUsd(yourRolloverEth * ethPrice)}</div>
          </div>
          <div style={{ background: '#eaf3de', padding: '14px', borderRadius: '8px' }}>
            ...
            <div style={{ fontSize: '22px', fontWeight: 500, color: '#173404' }}>{fmtEth(yourBonusEth)}</div>
            <div style={{ fontSize: '13px', color: '#3b6d11', marginTop: '2px' }}>{fmtUsd(yourBonusEth * ethPrice)}</div>
          </div>
        </div>
        ...
        <div style={{ ...card, marginBottom: '12px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', gap: '12px' }}>
            ...
            <span style={{ fontSize: '16px', color: '#5f5e5a' }}>{fmtUsd(totalUsd)}</span>
          </div>
        </div>

        <div style={card}>
          <div style={{ fontSize: '12px', color: '#5f5e5a', marginBottom: '6px' }}>
            Estimated winner tokens received <span style={{ fontSize: '11px', color: '#888780' }}>(zero-slippage approximation)</span>
          </div>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: '12px' }}>
            <span style={{ fontSize: '18px', fontWeight: 500 }}>{fmtTokens(winnerTokens)}</span>
            <span style={{ fontSize: '13px', color: '#5f5e5a' }}>({winnerPct.toFixed(2)}% of supply)</span>
          </div>
        </div>
      </div>
    </div>
